...
export const api={
  ...
  ,syncTaskWorkers:ids=>request(`/api/v1/sync-task-workers?taskIds=${ids.join(',')}`)
  ...
  ,skipTtlGovernanceDryRunAndStart:(id,version,reason)=>request(`/api/v1/ttl-governance-tasks/${id}/skip-dry-run-and-start`,{method:'POST',headers:{'Idempotency-Key':idempotencyKey(),'If-Match':String(version)},body:JSON.stringify({reason,confirmed:true})})
  ...
  ,skipCleanupGovernanceDryRunAndStart:(id,version,reason)=>request(`/api/v1/cleanup-governance-tasks/${id}/skip-dry-run-and-start`,{method:'POST',headers:{'Idempotency-Key':idempotencyKey(),'If-Match':String(version)},body:JSON.stringify({reason,confirmed:true})})
  ...
  ,updateAnalysisAgent:(id,version,data)=>request(`/api/v1/analysis-agents/${id}`,{method:'PUT',headers:{'Idempotency-Key':idempotencyKey(),'If-Match':String(version)},body:JSON.stringify(data)})
  ,analysisRuns:(params={})=>request(`/api/v1/analysis?${new URLSearchParams(Object.entries(params).filter(([,v])=>v!==undefined&&v!==''))}`)
  ,analysisRun:id=>request(`/api/v1/analysis/${id}`)
  ,analyze:data=>request('/api/v1/analysis',{method:'POST',headers:{'Idempotency-Key':idempotencyKey()},body:JSON.stringify(data)})
