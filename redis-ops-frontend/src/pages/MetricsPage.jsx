...
export default function MetricsPage(){
  ...
  const load=async()=>{try{const [assets,next]=await Promise.all([api.clusters({size:200}),api.collectorMetrics()]);const items=assets.items||[];const nodeEntries=await Promise.all(items.map(async cluster=>[String(cluster.id),await api.collectorNodes(cluster.id).catch(()=>[])]));setClusters(items);setNodes(Object.fromEntries(nodeEntries));setMetrics(previous=>{if(JSON.stringify(previous)!==JSON.stringify(next))setFlash(value=>value+1);return next});setError('')}catch(e){setError(e.message)}finally{setLoading(false)}}
