...
public class MyBatisIdempotencyRepository implements IdempotencyRepository {
    ...
    public MyBatisIdempotencyRepository(IdempotencyMapper mapper) {
        this.mapper = mapper;
    }
    @Override
    public boolean tryStart(String operator, String key, String operation, String digest) {
        try {
            return mapper.insert(operator, key, operation, digest) == 1;
    ...
    public Optional<IdempotencyRecord> find(String operator, String key) {
        return Optional.ofNullable(mapper.find(operator, key));
    }
    @Override
    public void complete(String operator, String key, String resourceId) {
        mapper.complete(operator, key, resourceId);
    }
    @Override
    public void fail(String operator, String key) {
        mapper.fail(operator, key);
    }
}
