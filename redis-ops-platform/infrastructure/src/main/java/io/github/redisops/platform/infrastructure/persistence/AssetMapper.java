...
import io.github.redisops.platform.domain.audit.AuditLog;
...
public interface AssetMapper {
    ...
    void insertApplication(ApplicationRow row);
    @Select("SELECT id,app_code AS code,name,owner,business_line,status,version FROM application WHERE id=#{id} AND deleted_at IS NULL")
    ManagedApplication findApplication(long id);
    @Select("SELECT id,app_code AS code,name,owner,business_line,status,version FROM application WHERE deleted_at IS NULL ORDER BY id DESC")
    ...
    void deleteNodes(long clusterId);
    @Insert("""
    ...
    @Options(useGeneratedKeys = true, keyProperty = "id")
    void startDiscovery(DiscoveryRow row);
    ...
    class ApplicationRow {
        public Long id;
        public String code, name, owner, businessLine, status;
        public long version;
    }
    class DiscoveryRow {
        public Long id;
