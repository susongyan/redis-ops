package io.github.redisops.platform.application.asset;
...
public class ClusterService {
    ...
    private final io.github.redisops.platform.domain.location.LocationRepository locations;
    ...
    public ClusterService(ClusterRepository clusters, AuditRepository audits,
            io.github.redisops.platform.domain.location.LocationRepository locations,
    ...
    public PageResult<RedisCluster> list(ClusterQuery query) {
        return clusters.findAll(query);
    }
    ...
    private void validateIdc(Long idcId) {
        if (idcId == null)
            throw new BusinessException("INVALID_ARGUMENT", "idcId is required");
        var idc = locations.findIdc(idcId).orElseThrow(() -> BusinessException.notFound("idc", idcId));
        if (idc.status() != io.github.redisops.platform.domain.location.ResourceStatus.ACTIVE)
    ...
    private static void validateAuthentication(UpsertCluster command, Optional<RedisClusterSecret> current,
            boolean creating) {
    ...
    private void applyAuthentication(long clusterId, UpsertCluster command, Optional<RedisClusterSecret> current,
            String operator) {
        if (!command.authEnabled()) {
            if (current.isPresent()) {
                secrets.deleteByClusterId(clusterId);
                audits.append(operator, "CLUSTER_CREDENTIAL_CLEAR", "REDIS_CLUSTER", Long.toString(clusterId),
                        "SUCCESS");
            }
            return;
        }
        ...
        if (command.password() == null || command.password().isBlank()) {
            ...
            if (!java.util.Objects.equals(existing.username(), username)) {
                ...
                audits.append(operator, "CLUSTER_CREDENTIAL_UPDATE", "REDIS_CLUSTER", Long.toString(clusterId),
                        "SUCCESS");
            }
            return;
        }
        ...
        try {
            ...
            secrets.save(new RedisClusterSecret(clusterId, uuid, encrypted.ciphertext(), encrypted.keyId(), username,
    ...
    public record AuthenticationSummary(boolean authEnabled, String username, String authType,
