...
当前版本 **V34**，验证环境 MySQL 8.4.11：54 张业务表、1 张 Flyway 历史表、20 条命令树节点种子数据（15 条命令、5 个分类）。
包含 Key 分布任务、运行槽和分组表，以及已有 Worker IP 字段。当前版本与 SHA256 以 manifest.json 为准。

## DBA 直接使用

执行 [redis-governance-init.sql](redis-governance-init.sql)，包含建库、完整表结构、必需种子数据和真实 V34 BASELINE。
...
预期 history 为一条 version=34、type=BASELINE、success=1，命令树节点数为20。
Platform 启动后 validate 通过，V1–V34 不再执行；未来 V35 起由 Flyway 增量升级。Worker 不执行 Flyway。
