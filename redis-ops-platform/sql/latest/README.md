...
当前版本 **V29**，验证环境 MySQL 8.4.11：43 张业务表、1 张 Flyway 历史表、15 条命令种子数据。
包含分析表和 `sync_runtime.worker_ip`、`worker_ip_runtime_id`。当前版本与 SHA256 以 manifest.json 为准。

## DBA 直接使用

执行 [redis-governance-init.sql](redis-governance-init.sql)，包含建库、完整表结构、必需种子数据和真实 V29 BASELINE。
...
预期 history 为一条 version=29、type=BASELINE、success=1，命令数为15。
Platform 启动后 validate 通过，V1–V29 不再执行；未来 V30 起由 Flyway 增量升级。Worker 不执行 Flyway。
