package io.github.redisops.platform.api.asset;

import io.github.redisops.platform.api.*;
import io.github.redisops.platform.application.asset.*;
...
import io.github.redisops.platform.domain.location.Idc;
...
public class ClusterController {
    ...
    public ApiResponse<RedisCluster> create(@RequestHeader("Idempotency-Key") String key,
            @Valid @RequestBody ClusterRequest body, HttpServletRequest request) {
        String operator = operator(request);
        RedisCluster result = idempotency.execute(operator, key, "CLUSTER_CREATE", body,
                () -> clusters.create(body.command(), operator), c -> c.id().toString(),
                id -> clusters.get(Long.parseLong(id)));
    ...
    @PutMapping("/{id}")
    public ApiResponse<RedisCluster> update(@PathVariable long id, @RequestHeader("If-Match") long version,
            @RequestHeader("Idempotency-Key") String key, @Valid @RequestBody ClusterRequest body,
            HttpServletRequest request) {
        String operator = operator(request);
        return wrap(idempotency.execute(operator, key, "CLUSTER_UPDATE",
    ...
    public void delete(@PathVariable long id, @RequestHeader("If-Match") long version,
        ...
        String operator = operator(request);
        idempotency.executeVoid(operator, key, "CLUSTER_DELETE",
                new MutationRequest(id, version, null), () -> clusters.delete(id, version, operator),
                Long.toString(id));
    }
    @PostMapping("/connection-tests")
    public ApiResponse<RedisConnectionTestResult> testConnection(@Valid @RequestBody ConnectionTestRequest body,
            HttpServletRequest request) {
    ...
    public ApiResponse<AssetService.DiscoverySubmission> discover(@PathVariable long id,
            @RequestHeader("Idempotency-Key") String key, HttpServletRequest request) {
        String operator = operator(request);
        return wrap(idempotency.execute(operator, key, "CLUSTER_DISCOVERY", id,
                () -> assets.enqueueDiscovery(id, operator, key), s -> s.job().id().toString(),
                jobId -> assets.getSubmission(Long.parseLong(jobId))), request);
    }
    @GetMapping("/{id}/discoveries")
    public ApiResponse<List<DiscoveryRun>> discoveries(@PathVariable long id, HttpServletRequest request) {
        return wrap(assets.discoveries(id), request);
    }
    @GetMapping("/{id}/nodes")
    public ApiResponse<List<RedisNode>> nodes(@PathVariable long id, HttpServletRequest request) {
        return wrap(assets.nodes(id), request);
    }

    public record ClusterRequest(@NotBlank String name, @NotBlank String environment, String businessLine,
            @NotBlank String owner, String opsOwner, String serviceLevel, @NotNull ClusterMode mode,
            String redisVersion,
            @NotBlank String endpoint, @NotNull Long idcId, boolean authEnabled, String username, String password,
            ClusterStatus status) {
        ClusterService.UpsertCluster command() {
            return new ClusterService.UpsertCluster(name, environment, businessLine, owner, opsOwner, serviceLevel,
                    mode, redisVersion, endpoint, idcId, authEnabled, username, password, status);
        }
    }
