...
public class AssetService {
    ...
    public List<ManagedApplication> listApplications() { return assets.findApplications(); }
    public ManagedApplication getApplication(long id){return assets.findApplication(id).orElseThrow(()->BusinessException.notFound("application",id));}
    @Transactional public ManagedApplication updateApplication(long id,long version,String code,String name,String owner,String businessLine,String status,String operator){
        getApplication(id);ManagedApplication replacement=new ManagedApplication(id,code,name,owner,businessLine,status==null?"ACTIVE":status,version);
        if(!assets.updateApplication(replacement,version))throw concurrent("application");audits.append(operator,"APPLICATION_UPDATE","APPLICATION",Long.toString(id),"SUCCESS");return getApplication(id);
    }
    @Transactional public void deleteApplication(long id,long version,String operator){
        getApplication(id);if(!assets.deleteApplication(id,version))throw concurrent("application");audits.append(operator,"APPLICATION_DELETE","APPLICATION",Long.toString(id),"SUCCESS");
    }
    public List<ApplicationBinding> applicationBindings(long id){getApplication(id);return assets.findBindingsByApplication(id);}

    @Transactional
    public void bind(long applicationId, ApplicationBinding binding, String operator) {
        assets.findApplication(applicationId).orElseThrow(() -> BusinessException.notFound("application", applicationId));
        clusters.findById(binding.clusterId()).orElseThrow(() -> BusinessException.notFound("cluster", binding.clusterId()));
        String poolConfig=binding.poolConfig()==null||binding.poolConfig().isBlank()?"{}":binding.poolConfig();
        try{if(!objectMapper.readTree(poolConfig).isObject())throw new IllegalArgumentException();}
        catch(Exception e){throw new BusinessException("INVALID_POOL_CONFIG","poolConfig must be a valid JSON object");}
    ...
    public List<ApplicationBinding> bindings(long clusterId) { return assets.findBindingsByCluster(clusterId); }
    public List<RedisNode> nodes(long clusterId) { return assets.findNodes(clusterId); }
    public List<DiscoveryRun> discoveries(long clusterId) { return assets.findDiscoveries(clusterId); }
    ...
    public DiscoverySubmission getSubmission(long jobId){
        ...
        DiscoveryRun run=assets.findDiscovery(job.bizId()).orElseThrow(()->BusinessException.notFound("discovery",job.bizId()));
        return new DiscoverySubmission(run,job);
    }
    public void executeDiscovery(long runId,long clusterId,String operator) {
        ...
        try {
            ...
            throw new BusinessException("DISCOVERY_FAILED", "topology discovery failed: " + abbreviate(exception.getMessage()));
        }
    }

    public record DiscoverySubmission(DiscoveryRun discovery,AsyncJob job){}
    private static BusinessException concurrent(String resource){return new BusinessException("CONCURRENT_MODIFICATION",resource+" was modified, reload and retry");}
