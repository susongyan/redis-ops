...
9. 源事务仅显式 v3 接纳，完整 EXEC 是提交边界；禁止混合范围过滤和跨 Slot 拆分事务。
   v1 / v2 不自动扩大范围。源帧、事务组装、待应用队列及 spool 回放均保持有界。
   按 ADR-026，契约 0.3.1 及对应 Worker 将 v3 扩展到 Redis 5.0；原始脚本仍阻塞，
   预检查和实际连接版本门禁保持一致，发布时不得混用尚未具备该能力的 Worker。
...
- ADR-031 将 Console 扩展为命令树及显式子命令通配，采用逐次普通/强化确认替代操作审批；精确禁用不回退通配，认领先持久化再执行，未知结果不自动重放。分阶段状态以 redis-operation-console.md 为准。
...
- 企业内部排障页面可保存并展示原始 Redis Key，以支持数据订正和慢命令定位；Key 不得进入日志、
  ...
  纳入资源级访问控制。
- TTL/清理治理按 ADR-027 支持两条路径：预检后审批执行，或填写原因并明确确认后跳过预检直接执行。
  两条路径都要求执行授权、影响上限、限速、暂停/取消及审计；跳过确认在同一事务内记录授权并入队。
  预检暂停只能通过预检恢复入口继续只读扫描，不能被普通执行恢复入口误用。删除优先 `UNLINK`。
