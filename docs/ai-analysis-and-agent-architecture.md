...
当前提供 HTTP/JSON 与自定义 ACP 适配器，配置 endpoint 后按请求调用，无分析启用开关。
分析与注册 API 始终可用；注册资料仅登记，不驱动运行时配置。模拟器不自动注册。
同类型 HTTP 优先于 ACP；没有匹配外部配置时使用 RULE 分析，结果明确标记来源。
配置修改仍需重启，详见 [配置说明](runtime-switches.md)。

调用链必须支持超时、取消、幂等、结果校验和降级。远程 Agent 不可用时直接返回分析失败，不隐式切换 Provider；AI 服务故障不能影响资产、同步、采集、风险和治理主流程。
