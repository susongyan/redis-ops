...
1. DBA 按[数据库手册](../redis-ops-platform/sql/README.md)准备数据库与账号；可以选择
   [V26 完整 SQL 初始化包](../redis-ops-platform/sql/baseline-v26/README.md)，或空库由 Flyway 逐条迁移，二选一。
   完整 SQL 已包含建库和 BASELINE，不再重复建库或执行 V1–V26；先不启动 Worker。
...
上线前必须配置网络访问限制、HTTPS、公司统一认证/授权与审计接入策略。
当前前端 `X-Operator` 来自本地浏览器值，不能当成可信身份认证；不要裸露业务写接口到公网。
