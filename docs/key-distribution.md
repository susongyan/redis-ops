...
结果抽屉支持 CSV 导出：任务结束（完成、不完整、失败或取消）后分页读取全部已保留分组，并校验版本未变化。UTF-8 BOM 文件包含任务预算、状态、覆盖说明、规则快照及计数误差，不包含原始 Key 样本或凭据。运行中或暂停中的任务不能导出。CSV 对公式前缀添加单引号保护；业务分组信息仅应分享给授权人员。Top-K / 容量超限不代表全部实际分组。
...
入口：采集与风险 → Key 分布分析。页面采用“选择范围 → 配置规则 → 确认执行”三步流程，支持返回修改并保留设置；任务列表始终独立显示在下方。
选择资产集群、DB，按需采样或手动输入示例，配置规则，确认预算后创建任务。
确认步骤分开显示统计分组数、扫描停止条件（数量/时长）和执行速率。固定容量模式标注“最多统计分组数”，Top-K 标注“候选分组数上限”，均不代表单次 SCAN 数量。
...
结果按任务自身的不可变规则快照显示“规则序号 · 名称 / 分组”，详情展示匹配前缀、提取方式、分隔符、取段数及完整规则 ID；不使用当前新建表单的规则解释历史结果。
...
distribution:
  ...
  max-scan-count: 5000         # 页面 COUNT 的部署上限
  min-scan-interval-ms: 10     # 页面扫描间隔的部署下限
  max-groups: 1000
...
```json
{
  ...
  "maxObservations":1000000, "durationSeconds":1800, "scanCount":200, "scanIntervalMillis":200
}
```
...
正式任务只使用 COUNT 和两次 SCAN 开始时间的最小间隔，不再按 Key/s 限速。默认 COUNT 200、间隔 200 ms；允许 COUNT 1–5000、间隔 10–60000 ms，部署配置可进一步收紧。间隔由整个任务共享，Cluster 各主分片不会分别获得额外额度。COUNT 是提示值，不保证返回数量；单页 1 MiB / 5000 个元素等安全上限保持不变。有限预览仍使用独立的保守采样预算。

连续三次批次往返超过 200 ms 将扫描间隔加倍（最多 60000 ms），连续六次暂停；三次连续超时暂停。它是扫描延迟保护，不是业务 P99 监控。
