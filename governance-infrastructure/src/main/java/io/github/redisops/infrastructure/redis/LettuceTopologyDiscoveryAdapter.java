...
public class LettuceTopologyDiscoveryAdapter implements TopologyDiscoveryPort, RedisConnectionTestPort {
    ...
    public LettuceTopologyDiscoveryAdapter(RedisConnectionProfileProvider profiles) { this.profiles=profiles; }

    @Override public List<RedisNode> discover(RedisCluster cluster) {
        try (RedisConnectionProfile profile = profiles.get(cluster.id())) {
            return discover(cluster.id(), profile);
        }
    }

    @Override public RedisConnectionTestResult test(ClusterMode mode, String endpoint, String username, char[] password) {
        ...
        try (RedisConnectionProfile profile = new RedisConnectionProfile(0, mode, parsed.seedEndpoints(),
            ...
            if ("INVALID_ENDPOINT".equals(exception.code())) throw exception;
    ...
    private List<RedisNode> discover(long clusterId, RedisConnectionProfile profile) {
        if(profile.mode()==ClusterMode.SENTINEL)return discoverSentinel(clusterId,profile);
        RuntimeException last=null;
        for(String endpoint:profile.seedEndpoints())try{return discoverSeed(clusterId,profile,HostPort.parse(endpoint));}catch(RuntimeException e){last=e;}
    ...
    private List<RedisNode> discoverSeed(long clusterId,RedisConnectionProfile profile,HostPort seed){
        ...
        try (StatefulRedisConnection<String,String> connection=client.connect()) {
            ...
            if (profile.mode()==ClusterMode.CLUSTER) return parseClusterNodes(clusterId,commands.clusterNodes());
            ...
            return List.of(new RedisNode(null,clusterId,seed.host(),seed.port(),server.get("run_id"),role,null,"[]",null,"CONNECTED"));
        } finally { client.shutdown(); }
    }

    private List<RedisNode> discoverSentinel(long clusterId,RedisConnectionProfile profile){
            RuntimeException last=null;
        for(String endpoint:profile.seedEndpoints())try{return discoverSentinelSeed(clusterId,profile,profile.sentinelMasterName(),HostPort.parse(endpoint));}catch(RuntimeException e){last=e;}
        throw new BusinessException("TOPOLOGY_UNREACHABLE",
                "all Sentinel seed endpoints failed" + (last == null ? "" : ": " + last.getMessage()), last);
    }

    private List<RedisNode> discoverSentinelSeed(long clusterId,RedisConnectionProfile profile,String masterName,HostPort seed){
        RedisURI uri=redisUri(seed,profile);
        RedisClient client=RedisClient.create(uri);
        try(StatefulRedisSentinelConnection<String,String> connection=client.connectSentinel(uri)){
            ...
            if(master==null||master.isEmpty())throw new BusinessException("EMPTY_TOPOLOGY","Sentinel master not found: "+masterName);
            List<RedisNode> result=new ArrayList<>();
            result.add(sentinelNode(clusterId,master,"MASTER",null));
            String masterId=master.get("runid");
            for(Map<String,String> replica:commands.replicas(masterName))result.add(sentinelNode(clusterId,replica,"REPLICA",masterId));
            return result;
        }finally{client.shutdown();}
    }

    private RedisURI redisUri(HostPort seed,RedisConnectionProfile profile){
        RedisURI.Builder builder=RedisURI.builder().withHost(seed.host()).withPort(seed.port()).withTimeout(Duration.ofSeconds(5));
        if (profile.password()!=null) {
            if (profile.username()!=null&&!profile.username().isBlank()) builder.withAuthentication(profile.username(),profile.password());
            else builder.withPassword(profile.password());
        }
    ...
    static List<RedisNode> parseClusterNodes(long clusterId,String raw) {
        ...
        for (String line:raw.split("\\R")) {
            ...
            String address=p[1].split("@",2)[0]; HostPort hp=HostPort.parse(address);
            ...
            nodes.add(new RedisNode(null,clusterId,hp.host(),hp.port(),p[0],role,"-".equals(p[3])?null:p[3],slots,null,
                    p[2].contains("fail")?"FAILED":"CONNECTED"));
        }
        if (nodes.isEmpty()) throw new BusinessException("EMPTY_TOPOLOGY","Redis returned no cluster nodes");
        return nodes;
    }
    private static Map<String,String> parseInfo(String raw) {
        Map<String,String> result=new HashMap<>();
        for(String line:raw.split("\\R")){int i=line.indexOf(':');if(i>0&&!line.startsWith("#"))result.put(line.substring(0,i),line.substring(i+1));}
        return result;
    }
    private static String toJson(String[] values) {
        StringJoiner joiner=new StringJoiner(",","[","]");
        for(String value:values) if(value.matches("[0-9]+(-[0-9]+)?")) joiner.add("\""+value+"\"");
    ...
    private static RedisNode sentinelNode(long clusterId,Map<String,String> values,String role,String masterId){
        ...
        String status=flags.contains("s_down")||flags.contains("o_down")||flags.contains("disconnected")?"FAILED":"CONNECTED";
    ...
    record HostPort(String host,int port) {
        static HostPort parse(String endpoint) {
            ...
            if(split<1) throw new BusinessException("INVALID_ENDPOINT","endpoint must be host:port");
            try{return new HostPort(value.substring(0,split).replace("[","").replace("]",""),Integer.parseInt(value.substring(split+1)));}
            catch(NumberFormatException e){throw new BusinessException("INVALID_ENDPOINT","endpoint port is invalid");}
        }
        static List<HostPort> parseAll(String endpoints){
            if(endpoints==null||endpoints.isBlank())throw new BusinessException("INVALID_ENDPOINT","at least one endpoint is required");
            return Arrays.stream(endpoints.split(",")).map(String::trim).filter(x->!x.isEmpty()).map(HostPort::parse).toList();
        }
    }
    record SentinelEndpoint(String masterName,List<HostPort> seeds){
        static SentinelEndpoint parse(String value){int at=value.indexOf('@');if(at<1)throw new BusinessException("INVALID_ENDPOINT","Sentinel endpoint must be masterName@host:port[,host:port]");return new SentinelEndpoint(value.substring(0,at),HostPort.parseAll(value.substring(at+1)));}
    }
}
