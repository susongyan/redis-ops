...
class AesGcmCredentialSecretProtectorTest {
    ...
    @Test void encryptsAndDecryptsWithoutEmbeddingPlaintext() {
    ...
    @Test void rejectsTamperingWrongKeyAndWrongCredentialAad() {
        UUID uuid = UUID.randomUUID();
    ...
    @Test void supportsDualKeyReadAndCurrentKeyRewrite() {
        UUID uuid = UUID.randomUUID();
    ...
    @Test void validatesKeyConfiguration() {
    ...
    private static byte[] sequence(int start) { byte[] value=new byte[32];for(int i=0;i<value.length;i++)value[i]=(byte)(start+i);return value; }
    private static boolean contains(byte[] source, byte[] target) {
        outer: for(int i=0;i<=source.length-target.length;i++){for(int j=0;j<target.length;j++)if(source[i+j]!=target[j])continue outer;return true;}return false;
    }
}
